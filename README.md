...
ログイン設定
すべての画面はログイン後にだけ利用できます。起動前に次の環境変数を設定してください。

APP_USERNAME=ログイン用ユーザー名
APP_PASSWORD=十分に長く推測されにくいパスワード

Railwayでは対象サービスの Variables に設定します。ログイン情報が未設定の場合、アプリは開放されず利用停止状態になります。
