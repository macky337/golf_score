...
def run():
        ...
        if st.form_submit_button("スコアを保存"):
            try:
                ...
                for score in scores:
                    ...
                    back_score_value = st.session_state[f"back_{score_id}"]
                    back_putt_value = st.session_state[f"putt_{score_id}"]
                    game_pt_value = st.session_state[f"game_pt_{score_id}"]
                    
                    # データを更新
                    supabase.table('score').update({
                        'back_score': back_score_value,
                        'back_putt': back_putt_value,
                        'back_game_pt': game_pt_value
    ...
    with col1:
        if st.button("フロントスコア入力へ戻る"):
            switch_page("フロントスコア入力")
    
    with col2:
        if st.button("エキストラスコア入力へ"):
            # ラウンドにエキストラホールがあるフラグを設定
            try:
