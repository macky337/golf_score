...
def run():
    supabase = get_supabase_client()    # タイトルとホームボタンのレイアウト
    ...
    st.markdown(
        ...
        /* 全ての最初の列（プレイヤー名）のスタイル - より広範囲な選択子 */
        [data-testid="stDataFrame"] *:first-child,
        [data-testid="stDataFrame"] table tr > *:first-child,
        [data-testid="stDataFrame"] table > * > tr > *:first-child,
        [data-testid="stDataFrame"] div[class*="table"] tr > *:first-child,
        [data-testid="stDataFrame"] div[class*="Table"] tr > *:first-child {
        ...
        /* Streamlitの内部構造に対応した選択子 */
        [data-testid="stDataFrame"] div div div div div table tr td:first-child,
        ...
        div[data-testid="stDataFrame"] div[data-testid="stTable"] table tr td:first-child,
        div[data-testid="stDataFrame"] div[data-testid="stTable"] table tr th:first-child {
            color: #777777 !important;
            background-color: rgba(240, 242, 246, 0.9) !important;
            font-weight: 600 !important;
        }
        ...
        .stApp[data-theme="dark"] [data-testid="stDataFrame"] *:first-child {
            color: #cccccc !important;
            background-color: rgba(50, 50, 50, 0.9) !important;
        }
        
        .stApp[data-theme="light"] [data-testid="stDataFrame"] *:first-child {
        ...
        [data-testid="stDataFrame"] * {
            --first-column-color: #777777;
            --first-column-bg: rgba(240, 242, 246, 0.9);
        }
        
        [data-testid="stDataFrame"] *:first-child {
            color: var(--first-column-color) !important;
            background-color: var(--first-column-bg) !important;
            font-weight: 600 !important;
        }
        </style>
        """,
    ...
    m_df = match_matrix_reset.astype(str)    styled_matrix = m_df.style.map(color_points)
    
    # プレイヤー列の文字色を灰色に設定
    styled_matrix = styled_matrix.set_properties(subset=['Player'], **{'color': '#666666', 'font-weight': 'bold'})
    ...
    r_df = df_reset.astype(str)    styled_results = r_df.style.map(color_points)
    
    # プレイヤー列の文字色を灰色に設定
    styled_results = styled_results.set_properties(subset=['Player'], **{'color': '#666666', 'font-weight': 'bold'})
