...
from modules.data_formatter import highlight_total_only, color_points, initialize_player_data
...
def run():
    # ナビゲーションバーを表示
    show_navigation(active_page="結果確認")
    ...
    # ラウンド選択
    rounds = supabase.table('rounds').select('round_id', 'date_played', 'course_name').order('date_played', desc=True).execute()
    if not rounds.data:
        st.warning("ラウンドデータがありません。")
        return
    
    # デフォルトでアクティブラウンドを選択した状態にする
    default_index = 0
    round_options = []
    for i, r in enumerate(rounds.data):
        round_options.append((r['round_id'], f"{r['date_played']} - {r['course_name']}"))
        if r['round_id'] == active_round_id:
            default_index = i
    ...
    selected_round_id = selected_round[0]
    
    # 選択したラウンドをアクティブにするオプション
    if selected_round_id != active_round_id:
        if st.button("このラウンドをアクティブにする"):
            st.session_state.active_round_id = selected_round_id
            # ラウンド情報を取得してhas_extraを確認
            round_info = supabase.table('rounds').select('has_extra').eq('round_id', selected_round_id).execute()
            if round_info.data and len(round_info.data) > 0:
                st.session_state.has_extra = round_info.data[0].get('has_extra', False)
            st.rerun()
    
    # スコア取得
    try:
        scores = get_scores_with_fallback(selected_round_id)
        if not scores:
    ...
    round_results = get_round_results(selected_round_id)
    ...
    round_result = supabase.table('rounds').select('*').eq('round_id', selected_round_id).execute()
    ...
    # ハンディキャップ情報の取得
    handicaps_result = supabase.table('handicap_match').select('*').eq('round_id', selected_round_id).execute()
    ...
    if not active_round['finalized']:
        if st.button("このラウンドを確定する"):
            try:
                ...
                if save_round_results(selected_round_id, updated_player_data):
                    update_data = {}
                    ...
                    success, updates, failures = update_scores_batch(selected_round_id, update_data)
                    if success:
                        st.success("ラウンドを確定しました")
                        supabase.table('rounds').update({'finalized': True}).eq('round_id', selected_round_id).execute()
                        if updates:
                            st.rerun()
                    else:
                    ...
                    st.session_state.calculation_results[selected_round_id] = updated_player_data
                    st.info("計算結果はセッションに保存されました。再試行してください。")
            except Exception as e:
                st.error(f"ラウンドの確定中にエラーが発生しました: {str(e)}")
                if "calculation_results" not in st.session_state:
                    st.session_state.calculation_results = {}
                st.session_state.calculation_results[selected_round_id] = player_data
                st.info("計算結果はセッションに保存されました。再試行してください。")
    ...
    if st.button("このラウンドの計算詳細をテスト表示", help="現在のロジックでこのラウンドを再計算し、詳細を表示します"):
        test_calculation_logic(selected_round_id)
...
def test_calculation_logic(round_id: int):
    ...
    with st.expander("計算結果の詳細を表示"):
        ...
        for pid in player_ids:
            ...
            st.write(f"  - Putt Front: {data['Putt Front']} / Putt Back: {data['Putt Back']} / Extra Putt: {data['Putt Extra']} => **Putt Pt: {data['Putt Pt']}**")
